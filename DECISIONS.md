...
- `roadmap` (enabled): `mark_done` + `mark_undone` + `status` — the agent NAMES completed/reopened checklist items; the server fuzzy-matches them against the canonical root `roadmap.md` and flips only those checkboxes (atomic write). **Why a tool, not markdown editing:** models (esp. Gemma 4) write the wrong file (`output/roadmap.md`), destructively rewrite the checklist, or invent invalid syntax (`- [in_progress]`). **Current ownership:** the main `compliance` agent OWNS the roadmap tools (not denied) and is told to call them inline. Because weak local models intermittently skip the roadmap tool, `/api/chat/stream` ALSO auto-fires a hidden roadmap-sync turn after the main turn — to the SAME `compliance` agent (no separate subagent), output suppressed (`inSubagentPhase`), backstopped by a 20s watchdog for empty turns. Visible feedback = progress bar + "Roadmap updated" chip only. This replaced both a background fire-and-forget sync (raced with notify turns) and a PROGRESS-tag approach (Qwen ignored it).
